...
func TestNoRecorderIsFine(t *testing.T) {
	// Tuned before Run rather than assigned after it. Writing to deps while
	// the session goroutine is live is a data race — cleanup reads deps.Calls
	// from the other side — and -race only ever missed it because the window
	// is wide. Same mistake as the injected clock in the afterhours test.
	h := startTuned(t, testPolicy, "512-555-0100", nil, func(d *Deps) { d.Calls = nil })
	h.finishPlayback(t)
	h.fake.expect(t, "CreateBridge")
	h.fake.expect(t, "AddToBridge") // the caller
	h.fake.expect(t, "Ring")
