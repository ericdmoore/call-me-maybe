...
func Env() *Schema {
	props := map[string]*Schema{
		...
		"WEBHOOK_URL":              env("Enables the event webhook: doorman POSTs one JSON object when the house starts ringing and one when a call ends, so Home Assistant can announce \"call from Grandma\" on a speaker or flash a light. Empty (the default) means no webhook. The payload is an event — what happened, who, and how it ended — and never names a speaker, a media_player entity, or any routing: Home Assistant already models the house's devices and decides which of them hear it. Posted off the call path through a buffered channel, so a dead or slow endpoint can never delay a call; a full buffer drops events and counts them. Treat the value as a secret — a Home Assistant webhook id is the whole credential — which is why doorman only ever logs its host.", "url", ""),
