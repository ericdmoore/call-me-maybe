...
# the published checksums file, and installs it. On Debian/Ubuntu it then
# offers to apt-install Asterisk — asking first, and only when there is a
# terminal to ask on. It never touches systemd or any config; that is
# docs/RUNBOOK.md's job.
#
# callmemaybe.cc/install.sh is this file, published by `make site-assets`;
# the raw GitHub URL for main works too and is what the site copy is made from.
set -euo pipefail
