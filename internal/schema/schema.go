...
func handsetItem() *Schema {
	return &Schema{
		...
		Properties: map[string]*Schema{
			...
			"mac": {
				Type:        "string",
				Description: "The phone's hardware address, from the sticker under the battery or on the box. Any written form is accepted — aa:bb:cc:dd:ee:ff, AA-BB-CC-DD-EE-FF, aabbccddeeff — and normalised. With model, it lets `doorman render` write the phone's own configuration file and `doorman provision` hand it over, which is what turns adding a phone into two touches instead of a web form.",
				Pattern:     `^[0-9A-Fa-f]{2}([:.\- ]?[0-9A-Fa-f]{2}){5}$`,
				Rules:       []string{"Two handsets may not share a MAC.", "mac and model travel together: set both or neither."},
			},
			"model": {
				Type:        "string",
				Description: "Which provisioning template the phone gets. `doorman provision --models` lists the known ids (grandstream-wp826, grandstream-grp2601p, …); `doorman check` refuses an unknown one with a suggestion.",
				Pattern:     "^[a-z0-9][a-z0-9-]*$",
				Rules:       []string{"mac and model travel together: set both or neither.", "A known model without a template yet is accepted; `doorman render` says so and the phone keeps the manual path."},
			},
			"phonebook": {
				Type:        "array",
				Items:       &Schema{Type: "string", Pattern: "^[a-z0-9][a-z0-9_-]*$"},
				Description: "Which directories this phone shows: \"house\" (every handset with a number and the feature codes), \"people\" (policy.toml's [[people]]), or a contacts.toml source id — a whole address book, which is why it is opt-in per phone. Absent means house and people. A child's phone might be just [\"house\"].",
				Rules:       []string{"Entries may not repeat.", "A contacts.toml source id is checked by `doorman check` against that file; the loader only checks the shape."},
				CrossRefs:   []string{"policy.toml [[people]]", "contacts.toml [[sources]] id"},
			},
		},
		AdditionalProperties: falsy,
	}
...
func action() *Schema {
	return &Schema{
		...
		Properties: map[string]*Schema{
			...
			"reply":   {Type: "string", Description: "What the house says back, where the transport can carry one.", Rules: []string{"Plain ASCII, at most 160 characters, no phone numbers, no links, no exclamation marks.", "An action needs a webhook, a reply, or both."}},
...
func Messages() *Schema {
	return &Schema{
		...
		Rules: []string{
			...
			"Replies are boring on purpose: plain ASCII, under 160 characters, no phone numbers, no links, no exclamation marks. One segment on the bill and past the carrier's spam filter, which dropped the first text this house ever sent. The loader refuses a reply that breaks the rule.",
			...
			"words": {
				...
				Items: &Schema{
					...
					Properties: map[string]*Schema{
						...
						"reply":   {Type: "string", Description: "Texted back to the sender. Optional.", Rules: []string{"Plain ASCII, at most 160 characters, no phone numbers, no links, no exclamation marks.", "A word needs a webhook, a reply, or both."}},
					},
					AdditionalProperties: falsy,
				},
			},
