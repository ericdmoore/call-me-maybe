...
| **s21** | [A phone's own voicemail](s21-a-phones-own-voicemail/) — `mailbox` on a handset becomes a real box render makes: a room call rings out into it, the phone's voicemail key opens it without a PIN, `init` gives every room one; adding a phone is a name, a number, an address book and a voicemail | M1–M3 shipped in v0.11.0 (2026-09-25); s12 unblocked | — |
| **s22** | [The house mailbox](s22-the-house-mailbox/) — midbury@bullmoose.cc hears everything the house hears: voicemail by `externnotify` through a `MAIL_HOOK` that runs the bullmoose CLI with a send-only token, the carrier's text forwarding, the house's replies, and a morning digest over the journal; doorman never learns to send mail | M1–M3 live on jepsen 2026-09-25 (voicemail, replies, digest — all read back from the mailbox); the carrier's text forwarding is a portal switch for the user; M4 the phone book waits on a decision | the mailbox and the CLI on the box (user); s21 for the room in the subject; the phone book is a bonus |
