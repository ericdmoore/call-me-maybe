...
- [ ] **The alert path must not be the phone.** If the balance is zero an
      ...
      and the notification leaves by another route. (`.plans/s03` M2 settles
      this differently and better: an *internal* call never touches a trunk, so
      a dead account can still ring the kitchen. Either way, not this
      milestone.)
