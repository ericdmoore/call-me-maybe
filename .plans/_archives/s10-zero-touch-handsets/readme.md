...
**Status: closed 2026-09-23 — archived.** M1–M6 shipped in v0.6.0, all
under test; the live rehearsal is the one thing left, and it is deliberately
scheduled with the jepsen re-provision rather than before it:

- M1: `mac`, `model`, `phonebook` on a handset; the model registry with
  did-you-mean; `HANDSET_<ID>_ADMIN_PASSWORD` and `_PROVISION_PASSWORD`
  generated by `init`, never typed (`080263e`).
- M2: `render` writes `cfg<mac>.xml` per Grandstream phone under
  `provisioning/`, 0600, from `PROVISION_ADDRESS` (`af70b6a`).
- M3: `doorman provision` — the inventory view over ARI endpoint state; the
  session with a once-minted certificate, the MAC allow-list, first fetch
  open and every later one with the phone's own credential, the unknown-MAC
  hint, the live watch, exit 0/1/2; `internal/provision/serve` reachable
  from `cmd/doorman/provision.go` only, asserted by test (`bafca40`).
- M4: `pjsip_notify.conf` shipped; `rotate --phones` (the flag is `--phones`,
  because `--handsets` was already the path flag); `provision notify`
  (`4c7ede3`).
- M5: house / people / contacts-source books; Grandstream `AddressBook`
  per phone beside its configuration; `doorman provision directory` and
  `doorman-directory.service`, one port above the window (8444 by default),
  re-reading the inputs on change; the analyzer treats phonebook names as
  caller data (`4c7ede3`).
- M6: RUNBOOK "Add a handset" and "Rotate handset passwords", FIRST-BOOT's
  "Phones" step, the README hardware column, man page, llms.txt.

**Open on close — verified by the first re-provision of jepsen, not before:**

1. The P-values marked VERIFY in `internal/provision/grandstream.go` (DTMF,
   codecs, upgrade mode, provisioning credentials, phonebook mode, timezone)
   against the kitchen phone's exported configuration, and the menu wording
   in `printInstructions` against the WP826's own screens.
2. That a WP826 honours `Event: check-sync` from `pjsip_notify.conf` without
   a reboot, and fetches on boot when the window is open.
3. Whether the phone accepts the self-signed certificate by default or needs
   `--export-cert`; and whether mDNS would have found the box (the discovery
   question this plan left to the rehearsal).
4. The `phonebook` refresh: a name added to `[[people]]` reaching both
   phones within one interval with no session opened.
